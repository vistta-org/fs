 ...
 * @param {Object} [options] - Options for the operation.
 * @returns {AsyncIterator<string | import("node:fs").Dirent>}
 */
 ...
 * @returns {boolean}
 */
export const isSymlink = (path) => {
  try {
    return statSync(path, false).isSymbolicLink();
  } catch {
    return false;
  }
};
 ...
 */
export const mktempFile = async (prefix = "/tmp") => {
  const dir = await mkdtemp(prefix);
  const filePath = resolve(dir, "file-" + Date.now());
 ...
 */
export const mktempFileSync = (prefix = "/tmp") => {
...
export const readdir = (path, options) =>
  _fs.promises
    ...
    .then((files) => (options?.withFileTypes ? files.map((dirent) => /** @type {any} */ (dirent).name) : files));
 ...
 *
 * @param {string | Buffer | URL} path - The path to the file.
 * @param {Object | string} [options] - Options for the operation, if options is a string, then it specifies the encoding.
 ...
 * @returns {string}
 */
export const relative = (from, to) => _path?.relative(from, to);
 ...
 * @returns {Promise<void>}
 */
export const rmdir = (path, options) => (options ? _fs.promises.rm(path, options) : _fs.promises.rmdir(path));
 ...
 * @returns {void}
 */
export const rmdirSync = (path, options) => (options ? _fs.rmSync(path, options) : _fs.rmdirSync(path));
 ...
 * @returns {string|null}
 */
export const resolveSymlink = (path) => {
  try {
 ...
 *
 * @param {string | Buffer | URL} path - The path to the file or directory.
 * @param {Object | ((curr: import("node:fs").Stats, prev: import("node:fs").Stats) => void)} [options] - Watch options or listener.
 * @param {(curr: import("node:fs").Stats, prev: import("node:fs").Stats) => void} [listener] - The listener function.
 * @returns {void}
 */
export const unwatchFile = (path, options, listener) => {
  if (typeof options === "function") return _fs.unwatchFile(path, options);
  if (typeof listener === "function") return _fs.unwatchFile(path, listener);
  return _fs.unwatchFile(path);
};
 ...
 *
 * @param {string | Buffer | URL} path - The path to the file or directory.
 * @param {Object | ((curr: import("node:fs").Stats, prev: import("node:fs").Stats) => void)} [options] - Options for the operation or listener.
 * @param {(curr: import("node:fs").Stats, prev: import("node:fs").Stats) => void} [listener] - The listener function.
 * @returns {import("node:fs").StatWatcher}
 */
export const watchFile = (path, options, listener) => {
  if (typeof options === "function") return _fs.watchFile(path, options);
  return _fs.watchFile(path, options, typeof listener === "function" ? listener : () => {});
};
...
export const dirname = (path) =>
  _path?.dirname(path instanceof URL || new RegExp("[^:]*://").test(path) ? fileURLToPath(path) : path);
 ...
 * @param {number} [length=12] - The length of the ID to return, defaults to 8 characters.
 * @returns {string | null} A unique ID or null if the file does not exist.
 */
export const fileId = (path, length = 12) => {
  if (!existsSync(path)) return null;
  const { birthtimeMs, size, mtimeMs } = statSync(path);
  return createHash("sha256").update(`${path}-${birthtimeMs}-${mtimeMs}-${size}`).digest("base64url").slice(0, length);
};
  ...
  return {
    [Symbol.asyncIterator]() {
      return {
        next: async () =>
          new Promise((resolve) =>
            setTimeout(() => watch(resolve(path), (filepath) => resolve({ value: filepath, done: false })), throttle),
          ),
      };
    },
  };
 ...
 * @param {Object|string|((event: string, filename: string | Buffer) => void)} [options] - Options, encoding, or listener.
 * @param {(event: string, filename: string | Buffer) => void} [listener] - Callback for change events.
 * @returns {import("node:fs").FSWatcher}
 */
export const watchNative = (path, options, listener) => {
  if (typeof options === "function") return _fs.watch(path, options);
  return _fs.watch(path, options, listener);
};
