...
if (typeof process !== "undefined") {
   ...
   */
  fs.access = (path, mode) => nodeFs?.promises?.access(path, mode);
   ...
   * @returns {void}
   */
  fs.accessSync = (path, mode) => nodeFs?.accessSync(path, mode);
   ...
   * @returns {Promise<void>}
   */
  fs.appendFile = (path, data, options) => nodeFs?.promises?.appendFile(path, data, options);
   ...
   * @returns {void}
   */
  fs.appendFileSync = (path, data, options) => nodeFs?.appendFileSync(path, data, options);
   ...
   * @returns {string}
   */
  fs.basename = (path, suffix) => nodePath?.basename(path, suffix);
   ...
   * @returns {Promise<void>}
   */
  fs.chmod = (path, mode) => nodeFs?.promises?.chmod(path, mode);
   ...
   * @returns {void}
   */
  fs.chmodSync = (path, mode) => nodeFs?.chmodSync(path, mode);
   ...
   * @returns {Promise<void>}
   */
  fs.chown = (path, uid, gid) => nodeFs?.promises?.chown(path, uid, gid);
   ...
   * @returns {void}
   */
  fs.chownSync = (path, uid, gid) => nodeFs?.chownSync(path, uid, gid);
   ...
   * @returns {void}
   */
  fs.closeSync = (fd) => nodeFs?.closeSync(fd);
   ...
   * @returns {Promise<void>}
   */
  fs.copyFile = (source, destination, mode) => nodeFs?.promises?.copyFile(source, destination, mode);
   ...
   * @returns {void}
   */
  fs.copyFileSync = (source, destination, mode) => nodeFs?.copyFileSync(source, destination, mode);
   ...
   * @returns {boolean}
   */
  fs.existsSync = (path) => nodeFs?.existsSync(path);
   ...
   * @returns {string}
   */
  fs.extname = (path) => nodePath?.extname(path);
   ...
   * @returns {void}
   */
  fs.fchmodSync = (fd, mode) => nodeFs?.fchmodSync(fd, mode);
   ...
   * @returns {void}
   */
  fs.fchownSync = (fd, uid, gid) => nodeFs?.fchownSync(fd, uid, gid);
   ...
   * @returns {void}
   */
  fs.fdatasyncSync = (fd) => nodeFs?.fdatasyncSync(fd);
   ...
   * @returns {string}
   */
  fs.filename = (url) => fileURLToPath(url);
   ...
   * @returns {Object}
   */
  fs.fstatSync = (fd, options) => nodeFs?.fstatSync(fd, options);
   ...
   * @returns {void}
   */
  fs.fsyncSync = (fd) => nodeFs?.fsyncSync(fd);
   ...
   * @returns {void}
   */
  fs.ftruncateSync = (fd, length) => nodeFs?.ftruncateSync(fd, length);
   ...
   * @returns {void}
   */
  fs.futimesSync = (fd, atime, mtime) => nodeFs?.futimesSync(fd, atime, mtime);
   ...
   */
  // @ts-ignore
  fs.glob = (pattern, options) => nodeFs?.promises?.glob(pattern, options);
   ...
   */
  // @ts-ignore
  fs.globSync = (pattern, options) => nodeFs?.globSync(pattern, options);
   ...
   * @returns {boolean}
   */
  fs.isAbsolute = (path) => nodePath?.isAbsolute(path);
   ...
   * @returns {Promise<void>}
   */
  fs.lchown = (path, uid, gid) => nodeFs?.promises?.lchown(path, uid, gid);
   ...
   * @returns {void}
   */
  fs.lchownSync = (path, uid, gid) => nodeFs?.lchownSync(path, uid, gid);
   ...
   * @returns {Promise<void>}
   */
  fs.lutimes = (path, atime, mtime) => nodeFs?.promises?.lutimes(path, atime, mtime);
   ...
   * @returns {void}
   */
  fs.lutimesSync = (path, atime, mtime) => nodeFs?.lutimesSync(path, atime, mtime);
   ...
   * @returns {Promise<void>}
   */
  fs.link = (existingPath, newPath) => nodeFs?.promises?.link(existingPath, newPath);
   ...
   * @returns {void}
   */
  fs.linkSync = (existingPath, newPath) => nodeFs?.linkSync(existingPath, newPath);
   ...
   * @returns {Promise<string>}
   */
  fs.mkdir = (path, options) => nodeFs?.promises?.mkdir(path, options);
   ...
   * @returns {string}
   */
  fs.mkdirSync = (path, options) => nodeFs?.mkdirSync(path, options);
   ...
   * @returns {Promise<string>}
   */
  fs.mkdtemp = (prefix, options) => nodeFs?.promises?.mkdtemp(prefix, options);
   ...
   */
  fs.open = (path, flags, mode) => nodeFs?.promises?.open(path, flags, mode);
   ...
   * @returns {Object}
   */
  fs.openSync = (path, flags, mode) => nodeFs?.openSync(path, flags, mode);
   ...
   */
  fs.opendir = (path, options) => nodeFs?.promises?.opendir(path, options);
   ...
   */
  fs.opendirSync = (path, options) => nodeFs?.opendirSync(path, options);
   ...
   */
  fs.readdir = (path, options) => nodeFs?.promises?.readdir(path, options);
   ...
   */
  fs.readdirSync = (path, options) => nodeFs?.readdirSync(path, options);
   ...
   */
  fs.readFile = (path, options) => nodeFs?.promises?.readFile(path, options);
   ...
   * @returns {Buffer}
   */
  fs.readFileSync = (path, options) => nodeFs?.readFileSync(path, options);
   ...
   * @returns {Promise<string>}
   */
  fs.readlink = (path, options) => nodeFs?.promises?.readlink(path, options);
   ...
   * @returns {string}
   */
  fs.readlinkSync = (path, options) => nodeFs?.readlinkSync(path, options);
   ...
   */
  fs.readSync = (fd, buffer, offset, length, position) => nodeFs?.readSync(fd, buffer, offset, length, position);
   ...
   */
  fs.readvSync = (fd, buffers, position) => nodeFs?.readvSync(fd, buffers, position);
   ...
   */
  fs.realpath = (path, options) => nodeFs?.promises?.realpath(path, options);
   ...
   */
  fs.realpathSync = (path, options) => nodeFs?.realpathSync(path, options);
   ...
   * @returns {string} Resolved path.
   */
  fs.realpathSync.native = (path, options) => nodeFs?.realpathSync?.native(path, options);
   ...
   * @returns {Promise<void>}
   */
  fs.rename = (oldPath, newPath) => nodeFs?.promises?.rename(oldPath, newPath);
   ...
   * @returns {void}
   */
  fs.renameSync = (oldPath, newPath) => nodeFs?.renameSync(oldPath, newPath);
   ...
   * @returns {string}
   */
  fs.resolve = (...paths) => nodePath?.resolve(...paths);
   ...
   * @returns {Promise<void>}
   */
  fs.rmdir = (path, options) => nodeFs?.promises?.rmdir(path, options);
   ...
   * @returns {void}
   */
  fs.rmdirSync = (path, options) => nodeFs?.rmdirSync(path, options);
  ...
  fs.stat = (path, options) => {
    if (options === true || options?.followLink) return nodeFs?.promises?.stat(path, options);
    return nodeFs?.promises?.lstat(path, options);
  }
  ...
  fs.statSync = (path, options) => {
    if (options === true || options?.followLink) return nodeFs?.statSync(path, options);
   ...
   * @returns {Promise<void>}
   */
  fs.symlink = (target, path, type) => nodeFs?.promises?.symlink(target, path, type);
   ...
   * @returns {void}
   */
  fs.symlinkSync = (target, path, type) => nodeFs?.symlinkSync(target, path, type);
   ...
   * @returns {Promise<void>}
   */
  fs.truncate = (path, length) => nodeFs?.promises?.truncate(path, length);
   ...
   * @returns {void}
   */
  fs.truncateSync = (path, length) => nodeFs?.truncateSync(path, length);
   ...
   */
  fs.unlink = (path) => nodeFs?.promises?.unlink(path);
   ...
   * @param {string | Buffer | URL} path - Path to the file.
   */
  fs.unlinkSync = (path) => nodeFs?.unlinkSync(path);
   ...
   * @returns {Promise<void>}
   */
  fs.utimes = (path, atime, mtime) => nodeFs?.promises?.utimes(path, atime, mtime);
   ...
   * @returns {void}
   */
  fs.utimesSync = (path, atime, mtime) => nodeFs?.utimesSync(path, atime, mtime);
   ...
   */
  fs.writeFile = (path, data, options) => nodeFs?.promises?.writeFile(path, data, options);
   ...
   * @param {Object} options - Options for writing the file, if options is a string, then it specifies the encoding.
   */
  fs.writeFileSync = (path, data, options) => nodeFs?.writeFileSync(path, data, options);
   ...
   * @returns {number} Buffer containing the file data.
   */
  fs.writeSync = (fd, buffer, offset, length, position) => nodeFs?.writeSync(fd, buffer, offset, length, position);
   ...
   * @returns {number} The number of bytes read.
   */
  fs.writevSync = (fd, buffers, position) => nodeFs?.writevSync(fd, buffers, position);
   ...
   * @return the directory name of a path or url. Similar to the Unix dirname command.
   */
  fs.dirname = (path) => nodePath?.dirname(path instanceof URL || new RegExp("[^:]*://").test(path) ? fileURLToPath(path) : path);
  ...
  fs.watch = (path, options = {}) => {
    ...
    const watch = async (p, cb) => {
      ...
      if (fs.isFile(p)) return fs.watchFile(p, (n, o) => cb(p, n, o));
      const dir = fs.isDirectory(p) && (await fs.readdir(p));
      for (let i = 0, len = dir?.length || 0; i < len; i++)
        watch(fs.resolve(p, dir[i]), cb);
    };
    return {
        ...
        return {
          next: async () =>
            new Promise((resolve) =>
              setTimeout(
                () =>
                  watch(fs.resolve(path), (filepath) =>
